...
class FxCore {
    ...
    async createProjectV3(inputs, ctx) {
        ...
        if (scratch === question_2.ScratchOptionNo.id) {
            ...
            if (question_1.BotFeatureIds.includes(feature)) {
                inputs.feature = feature;
                await workflow_1.runAction("teams-bot.add", context, inputs);
            }
            if (question_1.TabFeatureIds.includes(feature)) {
                inputs.feature = feature;
                await workflow_1.runAction("teams-tab.add", context, inputs);
            }
        }
        if (inputs.platform === teamsfx_api_1.Platform.VSCode) {
            await globalState_1.globalStateUpdate(automaticNpmInstall, true);
    ...
    async provisionResourcesV3(inputs, ctx) {
        ...
        const res = await workflow_1.runAction("fx.provision", context, inputs);
        if (res.isErr())
    ...
    async deployArtifactsV3(inputs, ctx) {
        ...
        context.projectSetting = ctx.projectSettings;
        context.tokenProvider = globalVars_1.TOOLS.tokenProvider;
        const res = await workflow_1.runAction("fx.deploy", context, inputs);
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        ctx.projectSettings = context.projectSetting;
        return teamsfx_api_1.ok(teamsfx_api_1.Void);
    ...
    async publishApplicationV3(inputs, ctx) {
        ...
        context.projectSetting = ctx.projectSettings;
        context.tokenProvider = globalVars_1.TOOLS.tokenProvider;
        const res = await workflow_1.runAction("app-manifest.publish", context, inputs);
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        ctx.projectSettings = context.projectSetting;
        return teamsfx_api_1.ok(teamsfx_api_1.Void);
    ...
    async executeUserTask(func, inputs) {
        if (globalVars_1.isV3()) {
            if (func.method === "addFeature") {
                const res = await this.addFeature(inputs);
                if (res.isErr())
                    return teamsfx_api_1.err(res.error);
    ...
    async addFeature(inputs, ctx) {
        ...
        let res;
        if (feature === "sql") {
            res = await workflow_1.runAction("sql.add", context, inputs);
        }
        else if (question_1.BotFeatureIds.includes(feature)) {
            res = await workflow_1.runAction("teams-bot.add", context, inputs);
        }
        else if (question_1.TabFeatureIds.includes(feature)) {
            res = await workflow_1.runAction("teams-tab.add", context, inputs);
        }
        else if (feature === question_1.CicdOptionItem.id) {
            res = await workflow_1.runAction("cicd.add", context, inputs);
        }
        else if (feature === question_1.ApiConnectionOptionItem.id) {
            res = await workflow_1.runAction("api-connector.add", context, inputs);
        }
        else {
            return teamsfx_api_1.err(new error_1.NotImplementedError(feature));
        }
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        ctx.projectSettings = context.projectSetting;
    ...
    async executeUserTaskV3(func, inputs, ctx) {
        if (func.method === "addFeature") {
            ...
            if (feature === "sql") {
                ...
                return teamsfx_api_1.err(new error_1.TaskNotSupportError(feature));
            }
            if (res.isErr())
                return teamsfx_api_1.err(res.error);
            ctx.projectSettings = context.projectSetting;
