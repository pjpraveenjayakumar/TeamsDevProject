...
exports.migrateArm = exports.needMigrateToArmAndMultiEnv = exports.addPathToGitignore = exports.ProjectMigratorMW = exports.ArmParameters = void 0;
...
const constants_3 = require("../../plugins/resource/localdebug/constants");
const constants_4 = require("../../plugins/resource/appstudio/constants");
...
async function migrateMultiEnv(projectPath, log) {
    ...
    const projectSettings = path_1.default.join(fxConfig, teamsfx_api_1.ProjectSettingsFileName);
    ...
    await fs_extra_1.default.copy(path_1.default.join(fx, "settings.json"), projectSettings);
...
async function removeExpiredFields(devState, devUserData) {
    ...
    if (await fs_extra_1.default.pathExists(devUserData)) {
        ...
        for (const [_, value] of Object.entries(constants_3.LocalDebugConfigKeys)) {
...
async function queryProjectStatus(fx) {
    ...
    const hasBotCapability = solutionSettings.capabilities.includes(question_1.BotOptionItem.id);
    const hasMessageExtensionCapability = solutionSettings.capabilities.includes(question_1.MessageExtensionItem.id);
    const isSPFx = plugins === null || plugins === void 0 ? void 0 : plugins.some((plugin) => plugin.name === constants_1.PluginNames.SPFX);
...
async function getMultiEnvFolders(projectPath) {
    ...
    const templateAppPackage = path_1.default.join(projectPath, "templates", teamsfx_api_1.AppPackageFolderName);
...
async function ensureProjectSettings(projectSettingPath, envDefaultPath) {
    ...
    await fs_extra_1.default.writeFile(projectSettingPath, JSON.stringify(settings, null, 4), {
        encoding: "UTF-8",
    });
}
...
async function generateArmTemplatesFiles(ctx) {
    ...
    if (!(await fs_extra_1.default.pathExists(path_1.default.join(fxConfig, parameterEnvFileName)))) {
        throw teamsfx_api_1.err(new teamsfx_api_1.SystemError(error_1.CoreSource, "GenerateArmTemplateFailed", `Failed to generate ${parameterEnvFileName} on migration`));
    }
}
