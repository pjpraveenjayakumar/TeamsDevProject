...
async function grantPermissionImpl(param, envState, envName, telemetryReporter, ui, m365TokenProvider, logProvider, platform, email) {
    var _a, _b;
    ...
    try {
        const result = await collaborationUtil_1.CollaborationUtil.getCurrentUserInfo(m365TokenProvider);
        if (result.isErr()) {
        ...
        const userInfo = await collaborationUtil_1.CollaborationUtil.getUserInfo(m365TokenProvider, email);
        if (!userInfo) {
...
async function grantPermission(param) {
    ...
    if (param.apiVersion === 1) {
        ...
        return grantPermissionImpl(param, envState, envName, telemetryReporter, ui, m365TokenProvider, logProvider, platform, email);
    }
    else {
