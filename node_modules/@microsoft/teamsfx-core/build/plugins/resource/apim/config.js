...
class ApimPluginConfig {
    constructor(config, envName) {
        if (!config) {
            throw error_1.BuildError(error_1.NoPluginConfig, constants_1.TeamsToolkitComponent.ApimPlugin, constants_1.ComponentRetryOperations[constants_1.TeamsToolkitComponent.ApimPlugin]);
        }
        this.config = config;
    ...
    getValue(key, namingRule) {
        ...
        if (namingRule && value) {
            ...
            if (message) {
                throw error_1.BuildError(error_1.InvalidConfigValue, constants_1.TeamsToolkitComponent.ApimPlugin, key, message);
            }
        }
        return value;
    }
    ...
    checkAndGet(key) {
        const value = error_1.AssertConfigNotEmpty(constants_1.TeamsToolkitComponent.ApimPlugin, key, this.getValue(key), this.envName);
        return value;
    }
}
...
class FunctionPluginConfig {
    ...
    checkAndGet(key) {
        return checkAndGetOtherPluginConfig(this.configOfOtherPlugins, constants_1.TeamsToolkitComponent.FunctionPlugin, key, this.envName);
    }
}
...
class AadPluginConfig {
    ...
    checkAndGet(key) {
        return checkAndGetOtherPluginConfig(this.configOfOtherPlugins, constants_1.TeamsToolkitComponent.AadPlugin, key, this.envName);
    }
}
